...
2022年度報告書で提案したDataOpsプロセスのオープンデータ利活用への有効性を証明する為、イベント参加者が体験しやすい「データの収集蓄積」「データの活用」「フィードバック」プロセスの観点で検証イベントを開催した。  

イベントの中で行われたディスカッションやアンケートの結果から、以下の改善が必要であることが判明した。  
1. 公開されている非構造化データの構造化  
2. オープンデータ公開の持続的なサイクルの実現  
3. 継続的なデータのメンテナンス  

### 1. 公開されている非構造化データの構造化

#### データの構造化及び構造化データのメンテナンスにかかる作業コストの低減

現状公開されているオープンデータはほぼPDF形式であることが実情である。  
その為、データ提供者の自治体職員はデータの構造化や構造化されたデータのメンテナンスにおける作業コストが肥大化しており、結果「データの収集蓄積」プロセスにおける「新規データの収集・加工」「収集済データの加工」に対して抵抗感を感じていると考えられる。実際に検証イベントで作業を体験したところ、フォーマットのばらつきが多く見られ、データの前処理に時間がかかることがわかった。  

データが構造化されていないことで作業コストが肥大化していることがデータ公開への足かせになっていると想定される為、以下の改善を施すことが有効ではないかと考える。  
...
公開されている非構造化データの構造化の改善により、データ利用者側にとっても扱いやすいデータとなることでデータの活用、フィードバックの活発化が期待でき、**2.オープンデータ公開の持続的なサイクルの実現** の実現につながると考えられる。
