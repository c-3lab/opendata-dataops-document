...
2022年度報告書で提案したDataOpsプロセスのオープンデータ利活用への有効性を証明する為、「データの収集蓄積」「データの活用」「フィードバック」プロセスの観点で検証イベントを開催した。  

その結果、以下の改善が必要であることが判明した。  
1. 公開されている非構造化データの構造化（データの構造化及び構造化データのメンテナンスにかかる作業コストの低減）  
2. オープンデータ公開の持続的なサイクルの実現（公開オープンデータ数の増加）  
3. 継続的なデータのメンテナンス（ニーズのあるデータの提供）  

現状公開されているオープンデータはほぼPDF形式であることが実情である。  
その為、データ提供者の自治体職員はデータの構造化や構造化されたデータのメンテナンスの作業コストが肥大化しており、結果「データの収集蓄積」プロセスにおける「新規データの収集・加工」「収集済データの加工」に対して抵抗感が出ていると考えられる。実際に検証イベントで作業を体験したところ、フォーマットのばらつきが多く見られ、扱いやすいデータではない（データの処理プロセスにおけるデータの前処理に時間がかかる）ことがわかった。  

データ構造化されていないことで作業コストが肥大化していることがデータ公開への足かせになっていると想定される為、以下の改善を施すことが有効ではないかと考える。  

デジタル庁はGIF(政府相互運用性フレームワーク)[^3]や自治体標準オープンデータセット[^4]を公開しており、データの構造化はこれをベースとしてユースケースによって拡張することを徹底することが重要である。ただし、データの構造化は少なからずオープンデータに精通した人の知見が必要となることが想定される為、データ提供者とデータ利用者の間を仲介する（データを構造化する）ステークホルダを設け、この対応を実施するプロセスが必要であると考える。  

したがって、データフォーマットは国が公開しているものに準拠すること、そしてオープンデータを構造化するプロセスを設けることが **1.公開されている非構造化データの構造化** の実現に必要である。  

また、生成AIのChatGPTを活用することが有効であることが検証イベントでわかった。作成したデータのレビューやデータ構造化をChatGPTで実施することでブレのないデータ構造でのデータ作成が可能になる。

この改善により、データ利用者側にとっても扱いやすいデータとなることでデータの活用、フィードバックの活発化が期待でき、**2.オープンデータ公開の持続的なサイクルの実現** の実現につながると考えられる。

よりニーズのあるデータを提供していく為には「フィードバック」プロセスは非常に重要な役割を持つ。ただし、フィードバックを取り込む側であるデータ提供者のモチベーションが低ければ持続的にデータを改善させていくことは難しいのではと考える。  
データ提供者のモチベーションを維持していく為には、評価 + 具体的な活用事例や「データの○○部分を修正しました」といったネガティブな要素を排除したコメントを投稿できる仕組みをつくることが **3.継続的なデータのメンテナンス** の実現に必要である。  
現在、OSSとして公開されているCKAN拡張機能（ckanext-feedback）[^5]はデータセットに対する評価や「要望、質問、宣伝、感謝」のフィードバックコメントを記載できるようになっている。フィードバックコメントに対する承認機能も有している為、これに上記の仕組みに該当する機能を追加することが効果的ではないかと考える。

以上のことから、1～3を改善することがDataOpsプロセスをデータ利活用促進に活用する為には必要であると考える。
...
[^3]: Digital Agency,Government of Japan, "政府相互運用性フレームワーク（GIF）" [オンライン]. Available: https://www.digital.go.jp/policies/data_strategy_government_interoperability_framework. [アクセス日: 20 02 2024].  
[^4]: Digital Agency,Government of Japan, "自治体標準オープンデータセット（正式版）" [オンライン]. Available: https://www.digital.go.jp/resources/open_data/municipal-standard-data-set-test. [アクセス日: 20 02 2024].  
[^5]: C3Lab, "ckanext-feedback" [オンライン]. Available: https://github.com/c-3lab/ckanext-feedback. [アクセス日: 26 02 2024].   
