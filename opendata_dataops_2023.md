...
その為、データ提供者の自治体職員は、データの構造化や構造化されたデータのメンテナンスにおける作業コストが肥大化している。  
結果として「データの収集蓄積」プロセスにおける「新規データの収集・加工」「収集済データの加工」に対して抵抗感を感じていると考えられる。  
実際に検証イベントで作業を体験したところ、フォーマットのばらつきが多く見られ、非構造化データの構造化作業に時間がかかることがわかった。  

前述の作業コストの肥大化がデータ公開への足かせになっていると考えられる。  
対策としては、デジタル庁が公開しているGIF(政府相互運用性フレームワーク)[^3]や自治体標準オープンデータセット[^4]の利用を徹底することが有効だと考える。  
また、データ構造がそのまま利用できない場合は、ユースケースに合わせた項目の拡張などを検討する。  

#### データの構造化及びメンテナンスにかかる作業コストの低減

データの構造化は作業コストが高いことに加え、オープンデータに精通した人の知見が必要となることが想定される。  
対策としては、データ提供者とデータ利用者の間を仲介する（データを構造化する）ステークホルダを設け、この対応を実施するプロセスをDataOpsプロセスに追加する必要があると考える。  
また、検証イベントを通じて、生成AIのChatGPTをデータ構造化に活用可能なことが確認できた。  
データの構造化や作成したデータのレビューなどをChatGPTで実施することで、作業コストをかけずにブレのないデータ構造でのデータ作成が可能になると考えた。  

### 2. オープンデータ公開の持続的なサイクルの実現

前述のデータ構造化作業を中心に、オープンデータの公開には多くの時間が必要になるという懸念がある。  
そのため、現状では非構造化データの割合が多く、以下のような悪循環となっている。  

1. 非構造化データの割合が多い為、データが扱いにくい
2. データが扱いにくい為、データ利用者が少ない
3. データ利用者が少ない為、フィードバックが少ない
4. フィードバックが少ない為、ニーズのあるデータが公開できない

このような悪循環の結果、持続的なデータ公開が難しく、オープンデータの数が中々増えない状況となっていると推測される。  

この悪循環は**1. 公開されている非構造化データの構造化**により改善することが可能である。  
構造化された扱いやすいデータを作業コストをかけずに公開できるようにすることでデータ公開が促進される。  
また、扱いやすいデータが増えることで、データ利用者におけるデータの活用及びフィードバックが活発化され、持続的なデータ公開の実現につながると考えられる。  

### 3. 継続的なデータのメンテナンス

データを有効に活用していく為にはニーズに応じたメンテナンスを継続的に実施する必要があるが、そのモチベーションを維持する為の仕組みが考えられていない。  
よりニーズのあるデータを提供していく為には「フィードバック」プロセスは非常に重要な役割を持つ。  
ただし、フィードバックを取り込む側であるデータ提供者のモチベーションが低ければ、持続的なメンテナンスが難しいと考えられる。  

データ提供者のモチベーションを維持していく為には、前述のデータ加工コストの低減に加え、評価や具体的な活用事例のフィードバックが有効な可能性が高い。  
また、オープンデータに修正が必要なケースの場合、修正依頼という形式ではなく、データの修正報告のような形式をとることで、モチベーション低下を防ぐことができると推測される。  
具体的な例としては「データの○○部分を修正しました」といったネガティブな要素を排除したコメントを投稿できる仕組みをつくるといった方法などが考えられる。  

この仕組みを実現する方法の一つとして、OSSとして公開されているCKAN拡張機能（ckanext-feedback）[^5]を活用する方法が挙げられる。  
CKAN拡張機能はデータセットに対する評価や「要望、質問、宣伝、感謝」のフィードバックコメントを記載できるようになっている。  
フィードバックコメントに対する承認機能も有している為、これに上記の仕組みに該当する機能を追加することが効果的ではないかと考える。  

## **４ まとめ**  
本報告書では、2022年度報告書で提案したDataOpsプロセスのオープンデータ利活用への有効性を証明する為、イベント参加者が体験しやすい「データの収集蓄積」「データの活用」「フィードバック」プロセスの観点で検証イベントを開催し、その中で得た成果や課題点を元に有効性の確認と改善案をまとめた。  

イベントの中で行われたディスカッションやアンケートの結果から、DataOpsプロセスを有効にデータ利活用につなげる為には、以下の改善が必要であることがわかった。  

- 公開されている非構造化データの構造化
- オープンデータ公開の持続的なサイクルの実現
- 継続的なデータのメンテナンス

それぞれの改善策と効果について、以下にまとめる。  

### 公開されている非構造化データの構造化

#### 改善策

本課題において改善が必要な要因は以下である。  

- データ提供者とデータ利用者にとって扱いやすいデータを公開すること
- データ提供者の作業コストを低減すること

この改善策として以下を提案する。  

- データフォーマットはデジタル庁が公開しているGIFや自治体標準オープンデータセットに準拠する
- オープンデータを構造化するプロセスをDataOpsプロセスに追加する
- データの構造化や作成したデータのレビューに生成AIのChatGPTを活用する

#### 改善による効果

- データ利用者にとって扱いやすいデータを提供することが可能となる
- データ提供者の作業コストを下げることでデータ公開に対する抵抗感を抑えることが可能となる
...
- データ提供者のデータ公開に対する抵抗感を少なくすること
- データ利用者にとって扱いやすいデータを公開すること

**公開されている非構造化データの構造化**の改善策を講じることにより、この要因を解消することが可能である。

#### 改善による効果

以下のような善循環のサイクルを回すことが可能となる。  

1. データ公開に対する抵抗感が緩和されることでデータ公開が促進される
2. データ利用者にとって扱いやすいデータが公開されることでデータ利用者が増加する
3. データ利用者が増加することでデータの活用が活性化する
4. データの活用が活性化することでフィードバックも活性化する
5. フィードバックが活性化することでよりニーズに合ったデータの提供が可能となる
...
- データ提供者のモチベーションを低下させないこと
- データ提供者のモチベーションを高くすること

この改善策として以下を提案する。  

- データ加工コストの低減
- 評価や具体的な活用事例のフィードバックを行う
- 修正依頼という形式ではなく、データの修正報告のようなネガティブな要素を排除したフィードバックを行う

#### 改善による効果

データ提供者のデータメンテナンスに対するモチベーションを維持し、ニーズに合ったデータを継続的に提供することが可能となる。  

### 結論

以上の改善により、DataOpsプロセスは以下のような善循環のサイクルを回すことが可能となり、DataOpsプロセスのデータ利活用促進への有効性が証明できる。  

1. データ提供者は GIF or 自治体標準オープンデータセット に準拠したデータを作成する
2. データ構造化プロセスを実施し、データを構造化した上でデータを公開する
3. データ利用者は構造化された扱いやすいデータを活用する
4. データ利用者はデータに対してネガティブ要素を含まないフィードバックを実施する
5. データ提供者はモチベーションを維持したままフィードバックを取込、ニーズのあるデータを提供し続ける
